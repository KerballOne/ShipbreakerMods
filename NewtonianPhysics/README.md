...
- **MaxVelocityMps** - caps how fast you can drift, in meters per second - also raises how fast your thrusters alone can accelerate you (stock's thrust speed is a separate, much lower limit from your overall speed cap, but this setting drives both). Stock's overall cap is 20; this mod raises it to 200 by default. Set to 0 for no cap at all - though ~200 m/s is a hard engine limit either way. The HUD speed readout only turns red within 1 m/s of whichever cap actually applies, instead of stock's fixed (and much lower) threshold.
